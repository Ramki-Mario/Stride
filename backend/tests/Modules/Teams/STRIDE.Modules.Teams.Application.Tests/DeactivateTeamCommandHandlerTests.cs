...
public sealed class DeactivateTeamCommandHandlerTests
{
    ...
    private DeactivateTeamCommandHandler Sut() => new(_repo, _audit, _currentUser);
    ...
    [Fact]
    public async Task Handle_ActiveTeam_ReturnsSuccess()
    {
        var tenantId = Guid.NewGuid();
        var team     = ActiveTeam(tenantId);
        _repo.GetByIdAsync(tenantId, team.Id, Arg.Any<CancellationToken>()).Returns(team);

        var result = await Sut().Handle(new DeactivateTeamCommand(tenantId, team.Id, Guid.NewGuid()), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_ActiveTeam_SavesChanges()
    {
        var tenantId = Guid.NewGuid();
        var team     = ActiveTeam(tenantId);
        _repo.GetByIdAsync(tenantId, team.Id, Arg.Any<CancellationToken>()).Returns(team);

        await Sut().Handle(new DeactivateTeamCommand(tenantId, team.Id, Guid.NewGuid()), CancellationToken.None);

        await _repo.Received(1).SaveChangesAsync(Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task Handle_ActiveTeam_FiresAuditLog()
    {
        var tenantId = Guid.NewGuid();
        var team     = ActiveTeam(tenantId);
        _repo.GetByIdAsync(tenantId, team.Id, Arg.Any<CancellationToken>()).Returns(team);
        ...
        _ = _audit.Received(1).LogAsync(Arg.Is<AuditLogEntry>(e => e.Action == AuditActions.TeamDeactivated));
    }
    ...
    public async Task Handle_TeamNotFound_ReturnsFailure()
    {
        ...
        var result = await Sut().Handle(
            new DeactivateTeamCommand(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid()),
    ...
    [Fact]
    public async Task Handle_AlreadyInactive_DoesNotSave()
    {
        var tenantId = Guid.NewGuid();
        var team     = InactiveTeam(tenantId);
        _repo.GetByIdAsync(tenantId, team.Id, Arg.Any<CancellationToken>()).Returns(team);
