...
public sealed class KitReturnedNotificationHandlerTests
{
    private readonly IEmailSender _emailSender = Substitute.For<IEmailSender>();
    private readonly IUserRoleQueryService _userRoles = Substitute.For<IUserRoleQueryService>();
    private readonly KitReturnedNotificationHandler _handler;

    private static readonly Guid TenantId = Guid.NewGuid();

    public KitReturnedNotificationHandlerTests()
    {
        var logger = Substitute.For<ILogger<KitReturnedNotificationHandler>>();
        _handler = new KitReturnedNotificationHandler(_emailSender, _userRoles, logger);
    }

    private static KitReturnedEvent AnEvent() => new(
    ...
    public async Task Handle_NoAdminUsers_SendsNoEmails()
    {
        ...
        await _handler.Handle(
            new DomainEventNotification<KitReturnedEvent>(e),
    ...
    public async Task Handle_WithAdminUsers_SendsEmailsToAllAdmins()
    {
        ...
        foreach (var email in emails)
        {
            await _emailSender.Received(1).SendAsync(
                email,
                Arg.Is<string>(s => s.Contains("Kit item returned")),
