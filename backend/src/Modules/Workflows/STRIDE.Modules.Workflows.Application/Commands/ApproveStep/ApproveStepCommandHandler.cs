    ...
    : IRequestHandler<ApproveStepCommand, Result>
{
    ...
    public async Task<Result> Handle(ApproveStepCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Role guard — only users who hold the required role may approve.
            if (step.RequiredRoleId.HasValue)
            {
                var hasRole = await _userRoles.UserHasRoleAsync(
                    request.ApprovedBy, step.RequiredRoleId.Value, instance.TenantId, cancellationToken);

                if (!hasRole)
                    return Result.Failure(
                        $"You do not hold the required role to approve step '{step.StepName}'.");
            }

            instance.ApproveStep(request.StepInstanceId, request.ApprovedBy, request.Comment);
            _instances.Update(instance);
            await _instances.SaveChangesAsync(cancellationToken);

            _logger.LogInformation(
                "Step {StepId} in instance {InstanceId} approved by user {UserId}",
                request.StepInstanceId, instance.Id, request.ApprovedBy);

            return Result.Success();
        }
        catch (WorkflowDomainException ex)
        {
            _logger.LogWarning(ex, "Domain rule violated during ApproveStep");
