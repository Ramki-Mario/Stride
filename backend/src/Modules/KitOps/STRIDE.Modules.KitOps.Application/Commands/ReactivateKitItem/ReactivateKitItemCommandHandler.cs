using MediatR;
using STRIDE.BuildingBlocks.Application.Abstractions;
using STRIDE.BuildingBlocks.Application.Results;
using STRIDE.Modules.KitOps.Domain.Exceptions;
using STRIDE.Modules.KitOps.Domain.Repositories;

namespace STRIDE.Modules.KitOps.Application.Commands.ReactivateKitItem;

internal sealed class ReactivateKitItemCommandHandler
    : IRequestHandler<ReactivateKitItemCommand, Result>
{
    private readonly IKitItemRepository _kitItems;
    private readonly IAuditLogger       _audit;
    private readonly ICurrentUser       _currentUser;

    public ReactivateKitItemCommandHandler(
        IKitItemRepository kitItems,
        IAuditLogger       audit,
        ICurrentUser       currentUser)
    {
        _kitItems    = kitItems;
        _audit       = audit;
        _currentUser = currentUser;
    }

    public async Task<Result> Handle(
        ReactivateKitItemCommand request, CancellationToken cancellationToken)
    {
        var item = await _kitItems.GetByIdAsync(request.KitItemId, cancellationToken);
        if (item is null)
            return Result.Failure($"Kit item '{request.KitItemId}' not found.");

        try
        {
            item.Reactivate(request.ReactivatedBy);
        }
        catch (KitOpsDomainException ex)
        {
            return Result.Failure(ex.Message);
        }

        await _kitItems.SaveChangesAsync(cancellationToken);

        _ = _audit.LogAsync(new AuditLogEntry(
            TenantId:     request.TenantId,
            ActorId:      request.ReactivatedBy,
            ActorEmail:   _currentUser.Email,
            Action:       AuditActions.KitItemReactivated,
            ResourceType: "KitItem",
            ResourceId:   item.Id));

        return Result.Success();
    }
}
